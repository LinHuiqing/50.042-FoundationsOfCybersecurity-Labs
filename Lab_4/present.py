...
def genRoundKeys(key):
    ...
    for i in range(FULLROUND):
        key = rol(key, 80-19, 80)
        key = (sbox[key >> 76] << 76) + (key & ((2 ** 76) - 1))
        key = key ^ (i+1 << 15)
        roundkeys[i+2] = key >> 16
    return roundkeys
...
def sBoxLayer(state, mode="encrypt"):
    output = 0
    for i in range(16):
        pos = i*4
        mask = 0xF
        ...
        if mode == "encrypt":
            ...
            nibble = sbox.index(word)
        else:
            raise ValueError("please input a valid mode (i.e. encrypt or decrypt)")
        output += nibble << pos
    return output


def pLayer(state, mode="encrypt"):
    output = 0
    for i in range(64):
        if mode == "encrypt":
            perm = pmt[i]
...
def present_inv_round(state, roundKey):
    state = pLayer(state, mode="decrypt")
    state = sBoxLayer(state, mode="decrypt")
    state = addRoundKey(state, roundKey)
    return state
